...
<div class="figure">
__FIG5__
<p class="caption"><b>Figure 5</b> BIC across K = 2…10 under the primary full-covariance specification (solid blue) and diagonal covariance (dashed orange). K = 7 (orange circle) has the lowest BIC among all numerically well-behaved solutions (1616.02). The K ≥ 8 solutions are numerically degenerate (shaded red region) because their log-likelihoods are positive (singular covariances) and are not interpretable as profile solutions. Source: <code>results/paper1_strengthening/k_extended_model_comparison.csv</code> and <code>results/paper1_strengthening/covariance_k_extended.csv</code></p>
</div>

<div class="caveat-box">
<p><b>K-selection decision (honest statement</b> K = 7 is the <i>lowest-BIC solution among numerically well-behaved fits</i> (K = 2…7) under the primary full-covariance specification (BIC = 1616.02). The diagonal-covariance specification produces an even lower BIC at K = 7 (477.83) and K ≥ 8 solutions are numerically degenerate (positive log-likelihoods, indicating singular covariance matrices). K = 7 was retained as the primary solution as the lowest-BIC well-behaved model under the primary full-covariance specification</p>
</div>
...
<h3>3.5 Selected Solution: K = 7 Profile Structure (Lowest BIC)</h3>
<p>The K = 7 solution under the primary full-covariance specification has seven profiles with sizes [281, 125, 108, 264, 245, 70, 73], summing to N = 1166. Directionally, three profiles (P0, P4, P6) have mean <i>z</i>(INT) > mean <i>z</i>(BE) and four (P1, P2, P3, P5) have mean <i>z</i>(BE) > mean <i>z</i>(INT). The profile means on the z-score scale are shown in Figure 3 and the z-score means by profile in Figure 4</p>

<div class="figure">
__FIG3__
<p class="caption"><b>Figure 3</b> Seven-profile K = 7 solution: profile-specific means on <i>z</i>(INT) and <i>z</i>(BE). Marker size is proportional to profile N. Red markers = z(INT) > z(BE) profiles (P0, P4, P6); blue markers = z(BE) > z(INT) profiles (P1, P2, P3, P5). The dashed black line is the equality reference (z(INT) = z(BE)). Source: <code>results/paper1_strengthening/task1_K_7/profile_parameters.csv</code</p>
</div>

<div class="figure">
__FIG4__
<p class="caption"><b>Figure 4</b> Profile-specific means on the z-score scale for z(INT) (blue) and z(BE) (orange). Profile sizes are shown in parentheses on the x-axis labels. The K = 7 solution shows substantial heterogeneity in both the level and direction of the configuration. Source: <code>results/paper1_strengthening/task1_K_7/profile_parameters.csv</code</p>
</div>

<div class="table-wrap">
<div class="table-title">Table 3. K = 7 profile descriptives (z-scores)</div>
<table>
...
<tbody>
<tr><td>P0</td><td>281</td><td>24.10%</td><td>+0.7550</td><td>+0.2945</td><td>+0.4605</td><td>z(INT) > z(BE)</td</tr>
<tr><td>P1</td><td>125</td><td>14.26%</td><td>−1.3009</td><td>−1.0750</td><td>−0.2259</td><td>z(BE) > z(INT)</td</tr>
<tr><td>P2</td><td>108</td><td>12.34%</td><td>+0.9171</td><td>+1.6066</td><td>−0.6894</td><td>z(BE) > z(INT)</td</tr>
<tr><td>P3</td><td>264</td><td>22.64%</td><td>−0.4799</td><td>−0.2546</td><td>−0.2253</td><td>z(BE) > z(INT)</td</tr>
<tr><td>P4</td><td>245</td><td>21.01%</td><td>+0.1424</td><td>+0.0571</td><td>+0.0853</td><td>z(INT) > z(BE)</td</tr>
<tr><td>P5</td><td>70</td><td>6.00%</td><td>−2.3229</td><td>−1.4011</td><td>−0.9218</td><td>z(BE) > z(INT)</td</tr>
<tr><td>P6</td><td>73</td><td>6.26%</td><td>+1.4498</td><td>+0.4030</td><td>+1.0468</td><td>z(INT) > z(BE)</td</tr>
<tr><th>Total</th><th>1166</th><th>100.00%</th><th>—</th><th>—</th><th>—</th><td>3+4</td</tr>
</tbody>
</table>
<p class="table-note">Profile sizes and z-scores verbatim from <code>results/paper1_strengthening/task1_K_7/profile_parameters.csv</code>. The K = 7 solution has the lowest BIC (1616.02) among all numerically well-behaved models.</p>
</div>
...
<div class="figure">
__FIG7__
<p class="caption"><b>Figure 7</b> Maximum posterior probability distribution for K = 7. Mean max posterior = 0.9105 (median 0.9995). The threshold lines show 0.70, 0.80, and 0.90. Most respondents are assigned with high confidence: 77.02% have max posterior ≥ 0.90, 82.16% ≥ 0.80, 84.91% ≥ 0.70, and 15.09% < 0.70. Source: <code>results/paper1_strengthening/k7_classification_quality.csv</code> and <code>results/paper1_strengthening/k7_posterior_probabilities.csv</code</p>
</div>

<div class="table-wrap">
<div class="table-title">Table 5. K = 7 classification-quality numerical summary (N = 1166</div>
<table>
<thead><tr><th>Statistic</th><th>Value</th</tr</thead>
<tbody>
<tr><td>Mean max posterior probability</td><td>0.9105</td</tr>
<tr><td>Median max posterior probability</td><td>0.9995</td</tr>
<tr><td>SD max posterior probability</td><td>0.1548</td</tr>
<tr><td>Min max posterior probability</td><td>0.5215</td</tr>
<tr><td>Max max posterior probability</td><td>1.0000</td</tr>
<tr><td><i>n</i> with max posterior ≥ 0.90</td><td>898</td</tr>
<tr><td><i>n</i> with max posterior ≥ 0.80</td><td>958</td</tr>
<tr><td><i>n</i> with max posterior ≥ 0.70</td><td>990</td</tr>
<tr><td><i>n</i> with max posterior < 0.70</td><td>176</td</tr>
<tr><td><i>n</i> with max posterior < 0.50</td><td>0</td</tr>
<tr><td>% max posterior ≥ 0.90</td><td>77.02%</td</tr>
<tr><td>% max posterior ≥ 0.80</td><td>82.16%</td</tr>
<tr><td>% max posterior ≥ 0.70</td><td>84.91%</td</tr>
<tr><td>% max posterior < 0.70</td><td>15.09%</td</tr>
<tr><td>% max posterior < 0.50</td><td>0.00%</td</tr>
</tbody>
</table>
<p class="table-note">All values verbatim from <code>results/paper1_strengthening/k7_classification_quality.csv</code>. The minimum observed max posterior is 0.5215 (above the 0.50 chance threshold for 7 equally-likely classes); 0 respondents have max posterior < 0.50</p>
</div>

<h3>3.8 Profile Stability</h3>
<div class="figure">
__FIG8__
<p class="caption"><b>Figure 8</b> Bootstrap directional stability for K = 7 across 200 bootstrap replications. Bars show the percentage of bootstraps in which the matched profile had mean z(INT) > mean z(BE). Two profiles (P0, P6) are ≥ 80% directionally stable; one profile (P2) is highly stable in the z(BE) > z(INT) direction (96.5%). Profile P4 is near chance (59.0%) and should be interpreted with caution. Source: <code>results/paper1_strengthening/k7_stability.csv</code</p>
</div>

<h3>3.9 Cross-Validation</h3>
<div class="figure">
__FIG10__
<p class="caption"><b>Figure 10</b> Five-fold cross-validation: mean held-out log-likelihood (±SD) across K = 2…7. K = 6 achieves the highest mean held-out log-likelihood (−287.58) among K = 2…7; however, K = 7 has the lowest BIC (1616.02) among numerically well-behaved models. K = 7 (−9.25, SD 162.37) shows a lower and higher-variance mean held-out log-likelihood, consistent with its numerical fragility under cross-validation despite its lowest BIC under the full-sample specification. Source: <code>results/paper1_strengthening/cv5_aggregated_by_k.csv</code</p>
</div>
...
<h3>3.10b Alternative-Specification Sensitivity (K = 7)</h3>
<p>To assess whether the K = 7 solution depends on specific modelling choices, the primary specification was compared against three alternative covariance structures (full / diagonal / spherical, all 10 indicators), two score representations (mean vs factor scores), and five random seeds (1–5). Table 6 reports the BIC, AIC, entropy, and total matching distance (sum of pairwise profile-centroid L2 distances under Hungarian matching) for each alternative</p>

<div class="table-wrap">
<div class="table-title">Table 6. K = 7 alternative-specification sensitivity (N = 1166; verbatim from <code>results/paper1_final/07_robustness/10_robustness_table.csv</code></div>
<table>
<thead><tr><th>Specification</th><th>BIC</th><th>AIC</th><th>Entropy</th><th>Total matching distance</th></tr></thead>
<tbody>
<tr style="background:#f0f8e8;"><td><b>Reference K = 7 (primary, full covariance, mean scores, seed 42)</b></td><td><b>1616.02</b></td><td><b>1408.51</b></td><td><b>1.0991</b></td><td><b>0.00</b></td</tr>
